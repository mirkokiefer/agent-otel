...
test('withPrivacy masks email in attributes', async () => {
  ...
  await wrapped.consume(mk({
    attributes: {
      'input.value': '{"to": "alex@example.com", "subject": "hi"}',
  ...
  expect(seen.attributes['input.value']).not.toContain('alex@example.com'); // masked
});
...
test('shared proxy maps same real PII to same fake across wrapped sinks', async () => {
  ...
  const span = mk({ attributes: { email: 'alex@example.com' } });
  ...
  expect(sinkA.spans[0]!.attributes.email).not.toBe('alex@example.com');
});
...
test('passthroughKeys skip masking on listed keys', async () => {
  ...
  await wrapped.consume(mk({
    attributes: {
      ...
      'input.value': 'Email alex@example.com',
    },
  }));
  ...
  expect(sink.spans[0]!.attributes['input.value']).not.toContain('alex@example.com');
});
...
test('events attributes also masked', async () => {
  ...
  await wrapped.consume(mk({
    events: [{
      name: 'tool.invoked',
      timeUnixNano: 100,
      attributes: { 'user.email': 'alex@example.com' },
    }],
  }));

  const eventAttrs = sink.spans[0]!.events[0]!.attributes!;
  expect(eventAttrs['user.email']).toBeString();
  expect(eventAttrs['user.email']).not.toBe('alex@example.com');
});
...
test('maskNames=true masks span name and status message', async () => {
  ...
  await wrapped.consume(mk({
    name: 'send email to alex@example.com',
    status: { code: 'ERROR', message: 'failed: alex@example.com bounced' },
  }));

  expect(sink.spans[0]!.name).not.toContain('alex@example.com');
  expect(sink.spans[0]!.status.message).not.toContain('alex@example.com');
});
...
test('end-to-end via Router: archive sees raw, vendor sees masked', async () => {
  ...
  await router.route(mk({
    attributes: {
      'gen_ai.system': 'anthropic',
      'input.value': '{"email":"alex@example.com","tracking":"AETH0000345323DY"}',
    },
  }));

  // Archive: raw values
  expect(archive.spans[0]!.attributes['input.value']).toContain('alex@example.com');
  expect(archive.spans[0]!.attributes['input.value']).toContain('AETH0000345323DY');
  // Vendor: fakes
  expect(vendor.spans[0]!.attributes['input.value']).not.toContain('alex@example.com');
...
test('maskSpan is pure (does not mutate input)', () => {
  ...
  const original = mk({ attributes: { email: 'alex@example.com' } });
