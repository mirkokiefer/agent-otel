...
🚧 v0.0.3 — APIs may change. Routing real production agent traffic at [Daslab](https://daslab.dev). MIT, no Daslab dependency.
...
**v0.0.3 — pre-alpha.** Core router, six reference sinks (memory/jsonl/otlp/phoenix/braintrust/slack), replay primitive (re-route flavor), reversible PII masking via `agent-otel/privacy` (composes with pii-proxy). Used by [Daslab](https://daslab.dev) to route production agent traces. API will change. Open issues, send PRs.
